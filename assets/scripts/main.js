...
function geoLocate() {
  ...
  function gotPosition(position) {
    ...
    const latlon = lat.toString() + "," + lon.toString();
    const dateObj = new Date();
    const todaysDate = dateObj.toISOString().split('.')[0]+"Z";
    const tomorrow = new Date(dateObj.setDate(new Date().getDate() + 1));
    const tomorrowsDate = tomorrow.toISOString().split('.')[0]+"Z";

    // use obj literal to concat lat & lon to url query
    //Original
    tmApiRequest = `https://app.ticketmaster.com/discovery/v2/events.json?latlong=${latlon}&radius=200&unit=miles&startDateTime=${todaysDate}&endDateTime=${tomorrowsDate}&apikey=0PYM69m0qo3ESz77SMGYGdnR0YZKo3oM`;
    // tmApiRequest = `https://app.ticketmaster.com/discovery/v2/events.json?latlong=${latlon}&radius=5&startDateTime=2021-04-13T11:55:00Z&endDateTime=2021-04-14T11:55:00Z&apikey=0PYM69m0qo3ESz77SMGYGdnR0YZKo3oM`;
    //Atttemped-failed
    //tmApiRequest = `https://upenn-cors-anywhere.herokuapp.com/https://app.ticketmaster.com/discovery/v2/events.json?latlon=${latlon}&radius=15&startDateTime=${todaysDate}&apikey=0PYM69m0qo3ESz77SMGYGdnR0YZKo3oM`;
    //Attempted-using "city=Philadelphia"
    //tmApiRequest = `https://upenn-cors-anywhere.herokuapp.com/https://app.ticketmaster.com/discovery/v2/events.json?city=philadelphia&radius=15&startLocalDate=2021-04-13&apikey=0PYM69m0qo3ESz77SMGYGdnR0YZKo3oM`;
    // Sam's call
